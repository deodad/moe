...
vacuum = next((thing for thing in state["things"] if "vacuum" in thing["name"].lower()), None)
...
metrics = {
    "model_insensitive_action_completed": float(vacuum is not None and len(bin_items) == 1 and bin_items[0]["thingId"] == vacuum["id"]),
