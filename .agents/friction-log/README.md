...
```sh
pnpm frog list    # what is already known
pnpm frog log     # add one
```

`pnpm frog log` writes the sections to fill in. Each id is when the friction was hit plus its title, so
